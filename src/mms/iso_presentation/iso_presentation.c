...
IsoPresentationIndication isoPres_setContextDefinition(IsoPresentation* self, int index, struct Member* member) {
	...
	self->context[index].contextId =
			member->presentationcontextidentifier;
...
static void createConnectPdu(IsoPresentation* self, ByteBuffer* buffer, ByteBuffer* payload) {
	...
	Contextlist_t* contextList = calloc(1, sizeof(Contextlist_t));
...
)
{
	...
	int pdvListEntries =
			cptype->normalmodeparameters->presentationcontextdefinitionlist->list.count;
			...
			== Userdata_PR_fullyencodeddata)
	{
		...
		self->nextContextId =
				pdvListEntry->presentationcontextidentifier;

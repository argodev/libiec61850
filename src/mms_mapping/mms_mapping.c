...
//	 */
	...
	if (attribute->subDataAttributes != NULL) {
		...
		int componentCount = ArrayList_listSize((void**) (attribute->subDataAttributes));
...
static MmsDevice* createMmsModelFromIedModel(IedModel* iedModel) {
	...
	if (iedModel->devices != NULL) {
		...
		int iedDeviceCount = ArrayList_listSize((void**) (iedModel->devices));
...
char* MmsMapping_createMmsVariableNameFromObjectReference(char* objectReference, FunctionalConstraint fc) {
	...
	char* mmsVariableName = malloc(objRefLength - i + 4);
